...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/01/21 15:59:12 by tsilberm          #+#    #+#             */
/*   Updated: 2016/01/21 16:51:13 by tsilberm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool		resolve(t_map *map, int t_ind)
{
	t_tetrimino		*t;
	...
	while (t->offset.y + t->height <= map->size)
	{
		while (t->offset.x + t->width <= map->size)
		{
			if (set(map, t))
			{
				...
				if ((t_ind + 1 >= map->t_count) || (resolve(map, t_ind + 1)))
					return (1);
				unset(map, t);
			}
			t->offset.x++;
